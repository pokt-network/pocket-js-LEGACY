 ...
 * @memberof Pocket
 */
  public async getSessionBlockHeight(useDefaultConfig = true): Promise<BigInt | RpcErrorResponse> {
    ...
    const sessionBlockHeightResponse = await RequestManager.getSessionBlockHeight(node, config)
 ...
 */
  public async getBlock(blockHeight: BigInt, useDefaultConfig = true): Promise<QueryBlockResponse | RpcErrorResponse> {
    const node = this.getAnyNode()
    let config = this.configuration
    // Check which configuration is being used
    if (!useDefaultConfig && this.optionalConfiguration != undefined) {
    ...
    const blockHeightResponse = await RequestManager.getBlock(blockHeight, node, config)
 ...
 * @memberof Pocket
 */
  public async getTX(txHash: String, blockHeight: BigInt, useDefaultConfig = true): Promise<QueryTXResponse | RpcErrorResponse> {
    const node = this.getAnyNode()
    let config = this.configuration
    // Check which configuration is being used
    if (!useDefaultConfig && this.optionalConfiguration != undefined) {
 ...
 * @memberof RequestManager
 */
  public async getHeight(useDefaultConfig = true): Promise<QueryHeightResponse | RpcErrorResponse> {
    const node = this.getAnyNode()
    let config = this.configuration
    // Check which configuration is being used
    if (!useDefaultConfig && this.optionalConfiguration != undefined) {
 ...
 * @memberof Pocket
 */
  public async getBalance(address: String, blockHeight: BigInt, useDefaultConfig = true): Promise<QueryBalanceResponse | RpcErrorResponse> {
    const node = this.getAnyNode()
    let config = this.configuration
    // Check which configuration is being used
    if (!useDefaultConfig && this.optionalConfiguration != undefined) {
    ...
    const balanceResponse = await RequestManager.getBalance(address, blockHeight, node, config)
 ...
 * @memberof RequestManager
 */
  public async getNodes(stakingStatus: StakingStatus, blockHeight: BigInt, useDefaultConfig = true): Promise<QueryNodesResponse | RpcErrorResponse> {
    const node = this.getAnyNode()
    let config = this.configuration
    // Check which configuration is being used
    if (!useDefaultConfig && this.optionalConfiguration != undefined) {
    ...
    const nodesResponse = await RequestManager.getNodes(stakingStatus, blockHeight, node, config)
 ...
 * @memberof Pocket
 */
  public async getNode(address: string, blockHeight: BigInt, useDefaultConfig = true): Promise<QueryNodeResponse | RpcErrorResponse> {
    const node = this.getAnyNode()
    let config = this.configuration
    // Check which configuration is being used
    if (!useDefaultConfig && this.optionalConfiguration != undefined) {
    ...
    const nodeResponse = await RequestManager.getNode(address, blockHeight, node, config)
 ...
 * @memberof Pocket
 */
  public async getNodeParams(blockHeight: BigInt, useDefaultConfig = true): Promise<QueryNodeParamsResponse | RpcErrorResponse> {
    const node = this.getAnyNode()
    let config = this.configuration
    // Check which configuration is being used
    if (!useDefaultConfig && this.optionalConfiguration != undefined) {
    ...
    const nodeParamsResponse = await RequestManager.getNodeParams(blockHeight, node, config)
 ...
 * @memberof Pocket
 */
  public async getNodeProofs(address: String, blockHeight: BigInt, useDefaultConfig = true): Promise<QueryNodeProofsResponse | RpcErrorResponse> {
    const node = this.getAnyNode()
    let config = this.configuration
    // Check which configuration is being used
    if (!useDefaultConfig && this.optionalConfiguration != undefined) {
    ...
    const nodeProofsResponse = await RequestManager.getNodeProofs(address, blockHeight, node, config)
 ...
 * @memberof RequestManager
 */
  public async getNodeProof(nodeProof: NodeProof, useDefaultConfig = true): Promise<QueryNodeProofResponse | RpcErrorResponse> {
    const node = this.getAnyNode()
    let config = this.configuration
    // Check which configuration is being used
    if (!useDefaultConfig && this.optionalConfiguration != undefined) {
    ...
    const nodeProofResponse = await RequestManager.getNodeProof(nodeProof, node, config)
 ...
 * @memberof Pocket
 */
  public async getApps(stakingStatus: StakingStatus, blockHeight: BigInt, useDefaultConfig = true): Promise<QueryAppsResponse | RpcErrorResponse> {
    const node = this.getAnyNode()
    let config = this.configuration
    // Check which configuration is being used
    if (!useDefaultConfig && this.optionalConfiguration != undefined) {
    ...
    const appsResponse = await RequestManager.getApps(stakingStatus, blockHeight, node, config)
 ...
 * @memberof Pocket
 */
  public async getApp(address: string, blockHeight: BigInt, useDefaultConfig = true): Promise<QueryAppResponse | RpcErrorResponse> {
    const node = this.getAnyNode()
    let config = this.configuration
    // Check which configuration is being used
    if (!useDefaultConfig && this.optionalConfiguration != undefined) {
    ...
    // Retrieve an app
    const appResponse = await RequestManager.getApp(address, blockHeight, node, config)
 ...
 * @memberof Pocket
 */
  public async getAppParams(blockHeight: BigInt, useDefaultConfig = true): Promise<QueryAppParamsResponse | RpcErrorResponse> {
    const node = this.getAnyNode()
    let config = this.configuration
    // Check which configuration is being used
    if (!useDefaultConfig && this.optionalConfiguration != undefined) {
    ...
    const appParamsResponse = await RequestManager.getAppParams(blockHeight, node, config)
 ...
 * @memberof Pocket
 */
  public async getPocketParams(blockHeight: BigInt, useDefaultConfig = true): Promise<QueryPocketParamsResponse | RpcErrorResponse> {
    const node = this.getAnyNode()
    let config = this.configuration
    // Check which configuration is being used
    if (!useDefaultConfig && this.optionalConfiguration != undefined) {
    ...
    const pocketParamsResponse = await RequestManager.getPocketParams(blockHeight, node, config)
 ...
 * @memberof Pocket
 */
  public async getSupportedChains(blockHeight: BigInt, useDefaultConfig = true): Promise<QuerySupportedChainsResponse | RpcErrorResponse> {
    const node = this.getAnyNode()
    let config = this.configuration
    // Check which configuration is being used
    if (!useDefaultConfig && this.optionalConfiguration != undefined) {
    ...
    const supportedChainsResponse = await RequestManager.getSupportedChains(blockHeight, node, config)
 ...
 * @memberof Pocket
 */
  public async getSupply(blockHeight: BigInt, useDefaultConfig = true): Promise<QuerySupplyResponse | RpcErrorResponse> {
    const node = this.getAnyNode()
    let config = this.configuration
    // Check which configuration is being used
    if (!useDefaultConfig && this.optionalConfiguration != undefined) {
    ...
    const supplyResponse = await RequestManager.getSupply(blockHeight, node, config)
